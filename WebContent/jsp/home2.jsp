...
<html lang="en">
  ...
  <body>
	...
	<%--userCount kısmında hata olabilir dikkat (userCount -1)--%>
	<% Integer userCount =  (Integer)request.getAttribute("tid") - 1;%>
	<% ArrayList<Kisi> users = (ArrayList<Kisi>)request.getAttribute("list"); %>
	<%for(Integer i = 0; i < userCount / 4; ){%>

	<div class="card-group">
	<%for(Integer j = userCount; j > userCount - 4 ; j--){%>
		<div class="card">
		  <img src="images/clyde.jpeg" class="card-img-top" alt="...">
		  <div class="card-body">
			<h5 class="card-title"><%= users.get(j).getAd() %></h5>
			<p class="card-text"><%= users.get(j).getEmail() %></p>
